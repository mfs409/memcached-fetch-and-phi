...
struct Config
{
    ...
    uint32_t    id_override;            /// use ID instead of rand numbers
    ...
    void usage(std::string name) {
        ...
        std::cerr << "    -I: use ID instead of rand number as parameter\n";
    ...
    void parseargs(int argc, char** argv, std::string name) {
        int opt;
        while ((opt = getopt(argc, argv, "N:d:p:hX:B:m:R:S:O:I")) != -1) {
            switch(opt) {
