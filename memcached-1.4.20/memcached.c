...
/*MYTAG: My forward declarations*/
static void process_bin_invoke(conn *c);
static void complete_bin_invoke(conn *c);
static void process_bin_register(conn *c);
static void complete_bin_register(conn *c);
...
extern pthread_mutex_t fcn_map_lock;

/*MYTAG: all connection objects are initilized here*/
/*
...
/*MYTAG: this is where new connections are initialized, including event handlers*/
...
/*MYTAG: the function that sets up the next state.*/
/*
...
/*MYTAG: reponses to binary commands handled here*/
...
/*MYTAG: Binary updates are handled here. ADD, REPLACE, APPEND, PREPEND, and SET*/
...
/*MYTAG: Binary gets are handled here*/
...
/*MYTAG: this is where keys are read for the binary protocol*/
...
static void dispatch_bin_command(conn *c) {
    ...
    switch (c->cmd) {
        ...
        case PROTOCOL_BINARY_CMD_NOOP:
            ...
            break;
	case PROTOCOL_BINARY_CMD_REG:
	    if(extlen == 0 && keylen > 0 && bodylen > 0) {
		bin_read_key(c, bin_reading_reg_header, extlen);		
	    } else {
		protocol_error = 1;
	    }
	    break;
	case PROTOCOL_BINARY_CMD_INV:
		//MYTAG TODO: read key?
	    if(extlen == 2 && keylen > 0) {
		bin_read_key(c, bin_reading_inv_header, extlen);
	    } else {
		protocol_error = 1;
	    }
	    break;
...
/*MYTAG: This is where requests using binary protocol are sent to be handled*/
static void complete_nread_binary(conn *c) {
    ...
    case bin_reading_set_header:
        ...
        break;
    case bin_reading_reg_header:
	process_bin_register(c);
	break;
    case bin_read_reg_value:
	complete_bin_register(c);
	break;
    case bin_read_set_value:
        complete_update_bin(c);
        break;
    case bin_reading_inv_header:
	process_bin_invoke(c);
	break;
    case bin_read_inv_value: //get the function key for the request	
        complete_bin_invoke(c);
	break;
...
/*MYTAG: our function for running scripts*/
static void process_script_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
	char *key;
	char *fcn_key;
	size_t fcn_key_len;
	size_t nkey;
	...
	//int i = 0;
	item *it;
	token_t *so_token = &tokens[SO_TOKEN];
	token_t *fcn_token = &tokens[FCN_TOKEN];
	token_t *key_token = &tokens[FCN_TOKEN+1];
	token_t *reply_token = NULL;
	if(ntokens >= 6) {
		reply_token = &tokens[FCN_TOKEN+2];
	}
	//char *suffix;
	assert(c != NULL);
	fcn_key_len = so_token->length + fcn_token->length + 2;
	//function keys look like [so path]:[function]
	fcn_key = (char*) malloc(fcn_key_len);
	strncpy(fcn_key, so_token->value, fcn_key_len);
	strncat(fcn_key, ":", 1);
	strncat(fcn_key, fcn_token->value, fcn_key_len);
	
	//pthread_mutex_lock(&fcn_map_lock);
	fcn = fcn_map_get(fm, fcn_key); //attempt to get fcn pointer
	if(!fcn) {
		//attempt to open so and store in map
		//if we ever fail, fall out of function
		void *handle = dlopen(so_token->value, RTLD_NOW);
		if(!handle) {
			out_string(c, dlerror());
			return;
		}

		dlerror(); //clear any open error
		*(void **)(&fcn) = dlsym(handle, fcn_token->value);
		...
		if(!fcn_map_set(fm, fcn_key, fcn)) {
			out_string(c, "FCN_NOT_FOUND");
			return;
		}
	}
	//pthread_mutex_unlock(&fcn_map_lock);
	free(fcn_key);
	...
	it = item_get(key, nkey);
	if(it != NULL) {	
		//"it" is a direct pointer to the item in cache
		char* buf = (char*)malloc(it->nbytes-2);	
		memcpy(buf, ITEM_data(it), it->nbytes-2);  //don't copy the \r\n
		int res = fcn(buf, it->nbytes-2, settings.item_size_max);
		item_remove(it);
		
		if(! item_size_ok(nkey, atoi(ITEM_suffix(it)), res+2)) {
			out_string(c, "SERVER_ERROR object too large for cache");
			if(buf) free(buf);
			return;
		}
	
		it = item_alloc(ITEM_key(it), it->nkey, atoi(ITEM_suffix(it)), it->exptime, res + 2);  //+2 to add CRLF
		if (it == 0) {
			if (! item_size_ok(nkey, atoi(ITEM_suffix(it)), res+2))
			    out_string(c, "SERVER_ERROR object too large for cache");
			else
		...
		ITEM_set_cas(it, (settings.use_cas) ? ITEM_get_cas(it) : 0);
		//item_remove(new_it);       /* release our reference */
		enum store_item_type ret = store_item(it, NREAD_REPLACE, c);
		if(ret == STORED) {
			if(reply_token && strcmp("noreply", reply_token->value) == 0) {
				out_string(c, "STORED");
			}
			else {
		...
		item_remove(it); //Free the item reference we are holding
		free(buf);
	}
	else {
		out_string(c, "NOT_FOUND");
...
static void process_command(conn *c, char *command) {
    ...
    /*MYTAG: here are the comparisons for checking the nature of the desired method*/
    ntokens = tokenize_command(command, tokens, MAX_TOKENS);
    if (ntokens >= 3 && strcmp(tokens[COMMAND_TOKEN].value, "mod") == 0) {
	process_script_command(c, tokens, ntokens, false); 
...
static int try_read_command(conn *c) {
	...
	/*MYTAG: Here's where the request is processed for binary protocol*/
    if (c->protocol == binary_prot) {
...
/*MYTAG: UDP request are read here*/
/*
 * read a UDP request.
 */
static enum try_read_result try_read_udp(conn *c) {
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_nread:
		/*MYTAG: reading all data starts here*/
            if (c->rlbytes == 0) {
		complete_nread(c);
                break;
            }
        ...
        case conn_mwrite:
		/*MYTAG: This is where server responses are handled*/
...
int main (int argc, char **argv) {
    ...
    /* Status messages */
    printf("Running memcached on port %d\n", settings.port);
...
/*MYTAG: called when all data for register has been received */
static void complete_bin_register(conn *c) {
	assert(c->item);
	int nkey = c->binary_header.request.keylen;
	int vlen = c->binary_header.request.bodylen - nkey;	
	...
	char fcn_key[nkey+1];
	memcpy(fcn_key, ITEM_key((item*)c->item), nkey);
	fcn_key[nkey] = 0x00;

	char pl[vlen+1]; //copy request body for now, add terminating character
	memcpy(pl, ITEM_data((item*)c->item), vlen);
	pl[vlen] = '\0';
	item_unlink(c->item);
	item_remove(c->item);
	c->item = 0;

	char *so_name, *fcn_name;
	char *save_ptr;
	so_name = strtok_r(pl, ":", &save_ptr);
	fcn_name = strtok_r(NULL, ":", &save_ptr);
	...
	//protolib.so won't go the first time with this
	if(!handle) {
		write_bin_error(c, PROTOCOL_BINARY_RESPONSE_NOT_STORED, dlerror(), 0);
	}
	else {
		dlerror(); //clear any open error
		*(void **)(&fcn) = dlsym(handle, fcn_name);

		const char *err = dlerror();
		if(err) {
			write_bin_error(c, PROTOCOL_BINARY_RESPONSE_EINVAL, err, 0);
		}
		else if(!fcn_map_set_n(fm, fcn_key, nkey, fcn)) {
			write_bin_error(c, PROTOCOL_BINARY_RESPONSE_NOT_STORED, NULL, 0);
		}
		else {
			//successful set, memcached automatically returns a default "success" response. Add in a response here if there are issues
			write_bin_response(c, NULL, 0, 0, 0);
		}
	}
	pthread_mutex_unlock(&fcn_map_lock);
...
/*MYTAG: called when all header information for register is received */
static void process_bin_register(conn *c) {
	int nkey = c->binary_header.request.keylen;
	int vlen = c->binary_header.request.bodylen - nkey;
	...
	c->rlbytes = vlen;
	conn_set_state(c, conn_nread);
	c->substate = bin_read_reg_value;
}

static void process_bin_invoke(conn *c) {
	protocol_binary_request_invoke *req = binary_get_request(c);
	uint16_t flags = ntohs(req->message.body.flags); //convert from network to host byte order
	...
	c->rlbytes = vlen;
	conn_set_state(c, conn_nread);
	c->substate = bin_read_inv_value;


	c->binary_request_flags = flags;
}

/** PSUEDOCODE: Line 6010 has the command commented out that causes a seg fault
complete_bin_invoke(connection c) 
	key = target_item_key();
	fcn_key = fcn_key_requested();

	if(cache[key] exists) 
		fcn = get_fcn(fcn_key);
		buf = DATA(cache[key]);

		result_size = fcn(buf, sizeof(buf));
		if(!item_size_ok(result_size + 2))
			EXIT(ERROR_VALUE);

		item = alloc_item(settings_for_previous_item, result_size);
		if(failed_item_alloc) 
			EXIT(ERROR_VALUE);

		DATA(item) = buf + '\r\n';
		
		stored_status = store_item(item, REPLACE_ITEM_WITH_SAME_KEY);
		if(stored_status is STORED) 
			write_response(DATA(item));
	else
		EXIT(ERROR_VALUE);		
END
*/

//now each body is in the form of DDDDDDDDD#functionkeyvalue....DDDDDDDDD#someserializedparameterstring..
//9 digits per number
static void complete_bin_invoke(conn *c) {
	int nkey = c->binary_header.request.keylen;
	//int vlen = c->binary_header.request.bodylen - nkey - c->binary_header.request.extlen;
	...
	char key[nkey+1];
	memcpy(key, ITEM_key((item*)c->item), nkey);
	key[nkey] = 0x00;

	char *fcn_data, *fcn_data_end; //copy request body for now, add terminating character
	fcn_data = ITEM_data((item*)c->item);

	long int fcn_key_len = strtol(fcn_data, &fcn_data_end, 10);
	char fcn_key[fcn_key_len+1];
	fcn_data_end++; //skip the '#'
	memcpy(fcn_key, fcn_data_end, fcn_key_len);
	fcn_key[fcn_key_len] = 0x00;

	fcn_data = fcn_data_end + fcn_key_len;
	long int fcn_param_len = strtol(fcn_data, &fcn_data_end, 10);
	char fcn_param[fcn_param_len+1];
	fcn_data_end++;
	memcpy(fcn_param, fcn_data_end, fcn_param_len);
	fcn_param[fcn_param_len] = 0x00;

	item_unlink(c->item); //c->item was used for reading in data from client, the .so path and function name in this implementation
	item_remove(c->item);
	c->item = 0;

	//item stored in c->item
	item* it = item_get(key, nkey); //this updates the refcount for the item, must be decremented later
	if(it != NULL) {	
		char buf[settings.item_size_max];  //[ams] allocating this large array on stack is probably a bad idea
		memcpy(buf, ITEM_data(it), it->nbytes-2);  //don't copy the \r\n

		//get item CAS
		uint64_t old_cas = ITEM_get_cas(it);
		fcn_handle fcn;
		pthread_mutex_lock(&fcn_map_lock);
		fcn = fcn_map_get_n(fm, fcn_key, fcn_key_len);
		pthread_mutex_unlock(&fcn_map_lock);
		if(!fcn) {
			write_bin_error(c, PROTOCOL_BINARY_RESPONSE_EINVAL, NULL, 0);
			return;
		}
		int res = fcn(buf, it->nbytes-2, settings.item_size_max); //process item with given function
		
		int suffix = (int) strtol(ITEM_suffix(it), (char **) NULL, 10);
		uint32_t exptime = it->exptime;	

		if(! item_size_ok(nkey, suffix, res+2)) {
			write_bin_error(c, PROTOCOL_BINARY_RESPONSE_E2BIG, NULL, 0);
			return;
		}
		...
		it = item_alloc(key, nkey, suffix, exptime, res + 2);  //create new item for result +2 to add CRLF
		if (it == 0) {
			write_bin_error(c, PROTOCOL_BINARY_RESPONSE_ENOMEM, NULL, 0);			    
			item_remove(it); 
			return;
		}
		memcpy(ITEM_data(it), buf, res); //buf copied into new item here.
		memcpy(ITEM_data(it) + res, "\r\n", 2);
		
		// Overwrite the older item's CAS with our new CAS since we're
		// returning the CAS of the old item below.
		ITEM_set_cas(it, (settings.use_cas) ? old_cas : 0);

		enum store_item_type ret = NOT_STORED;
		if(c->binary_request_flags&0x0010) {
			ret = STORED; //just to get through the following statement
		}
		else {
			ret = store_item(it, NREAD_CAS, c); //replace existing item
		}
		
		if(ret == STORED) {
			if((c->binary_request_flags)&0x0001) {
				item_remove(it);
				write_bin_response(c, NULL, 0, 0, 0);
				return;
			}
			else {
				c->item = it; //store item for transmitting back. Memcached derefences this later in conn_release_items.
				write_bin_response(c, ITEM_data(it), 0, 0, res); //Write item to client
				return;
			}
		}
		else {
			...
			item_remove(it);
			if(c->binary_request_flags&0x0010) {
				write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_EEXISTS, NULL, 0); //item not stored, key exists		
			}
			else {
				write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_EEXISTS, NULL, 0); //item not stored, key exists		
			}
		...
		write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_ENOENT, NULL, 0); //no item for given key
	}
}
