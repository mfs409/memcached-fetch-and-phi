// Generated by the protocol buffer compiler.  DO NOT EDIT!

#define INTERNAL_SUPPRESS_PROTOBUF_FIELD_DEPRECATION
#include "repeat_int.pb.h"

#include <algorithm>

#include <google/protobuf/stubs/once.h>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/wire_format_lite_inl.h>
#include <google/protobuf/descriptor.h>
#include <google/protobuf/reflection_ops.h>
#include <google/protobuf/wire_format.h>
// @@protoc_insertion_point(includes)

namespace test {

namespace {

const ::google::protobuf::Descriptor* Ints_descriptor_ = NULL;
const ::google::protobuf::internal::GeneratedMessageReflection*
  Ints_reflection_ = NULL;

}  // namespace


void protobuf_AssignDesc_repeat_5fint_2eproto() {
  protobuf_AddDesc_repeat_5fint_2eproto();
  const ::google::protobuf::FileDescriptor* file =
    ::google::protobuf::DescriptorPool::generated_pool()->FindFileByName(
      "repeat_int.proto");
  GOOGLE_CHECK(file != NULL);
  Ints_descriptor_ = file->message_type(0);
  static const int Ints_offsets_[2] = {
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(Ints, data_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(Ints, arr_),
  };
  Ints_reflection_ =
    new ::google::protobuf::internal::GeneratedMessageReflection(
      Ints_descriptor_,
      Ints::default_instance_,
      Ints_offsets_,
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(Ints, _has_bits_[0]),
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(Ints, _unknown_fields_),
      -1,
      ::google::protobuf::DescriptorPool::generated_pool(),
      ::google::protobuf::MessageFactory::generated_factory(),
      sizeof(Ints));
}

namespace {

GOOGLE_PROTOBUF_DECLARE_ONCE(protobuf_AssignDescriptors_once_);
inline void protobuf_AssignDescriptorsOnce() {
  ::google::protobuf::GoogleOnceInit(&protobuf_AssignDescriptors_once_,
                 &protobuf_AssignDesc_repeat_5fint_2eproto);
}

void protobuf_RegisterTypes(const ::std::string&) {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedMessage(
    Ints_descriptor_, &Ints::default_instance());
}

}  // namespace

void protobuf_ShutdownFile_repeat_5fint_2eproto() {
  delete Ints::default_instance_;
  delete Ints_reflection_;
}

void protobuf_AddDesc_repeat_5fint_2eproto() {
  static bool already_here = false;
  if (already_here) return;
  already_here = true;
  GOOGLE_PROTOBUF_VERIFY_VERSION;

  ::google::protobuf::DescriptorPool::InternalAddGeneratedFile(
    "\n\020repeat_int.proto\022\004test\"!\n\004Ints\022\014\n\004data"
    "\030\001 \001(\007\022\013\n\003arr\030\002 \003(\007", 59);
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedFile(
    "repeat_int.proto", &protobuf_RegisterTypes);
  Ints::default_instance_ = new Ints();
  Ints::default_instance_->InitAsDefaultInstance();
  ::google::protobuf::internal::OnShutdown(&protobuf_ShutdownFile_repeat_5fint_2eproto);
}

// Force AddDescriptors() to be called at static initialization time.
struct StaticDescriptorInitializer_repeat_5fint_2eproto {
  StaticDescriptorInitializer_repeat_5fint_2eproto() {
    protobuf_AddDesc_repeat_5fint_2eproto();
  }
} static_descriptor_initializer_repeat_5fint_2eproto_;


// ===================================================================

#ifndef _MSC_VER
const int Ints::kDataFieldNumber;
const int Ints::kArrFieldNumber;
#endif  // !_MSC_VER

Ints::Ints()
  : ::google::protobuf::Message() {
  SharedCtor();
}

void Ints::InitAsDefaultInstance() {
}

Ints::Ints(const Ints& from)
  : ::google::protobuf::Message() {
  SharedCtor();
  MergeFrom(from);
}

void Ints::SharedCtor() {
  _cached_size_ = 0;
  data_ = 0u;
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}

Ints::~Ints() {
  SharedDtor();
}

void Ints::SharedDtor() {
  if (this != default_instance_) {
  }
}

void Ints::SetCachedSize(int size) const {
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
}
const ::google::protobuf::Descriptor* Ints::descriptor() {
  protobuf_AssignDescriptorsOnce();
  return Ints_descriptor_;
}

const Ints& Ints::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_repeat_5fint_2eproto();  return *default_instance_;
}

Ints* Ints::default_instance_ = NULL;

Ints* Ints::New() const {
  return new Ints;
}

void Ints::Clear() {
  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    data_ = 0u;
  }
  arr_.Clear();
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
  mutable_unknown_fields()->Clear();
}

bool Ints::MergePartialFromCodedStream(
    ::google::protobuf::io::CodedInputStream* input) {
#define DO_(EXPRESSION) if (!(EXPRESSION)) return false
  ::google::protobuf::uint32 tag;
  while ((tag = input->ReadTag()) != 0) {
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      // optional fixed32 data = 1;
      case 1: {
        if (::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_FIXED32) {
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::uint32, ::google::protobuf::internal::WireFormatLite::TYPE_FIXED32>(
                 input, &data_)));
          set_has_data();
        } else {
          goto handle_uninterpreted;
        }
        if (input->ExpectTag(21)) goto parse_arr;
        break;
      }
      
      // repeated fixed32 arr = 2;
      case 2: {
        if (::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_FIXED32) {
         parse_arr:
          DO_((::google::protobuf::internal::WireFormatLite::ReadRepeatedPrimitive<
                   ::google::protobuf::uint32, ::google::protobuf::internal::WireFormatLite::TYPE_FIXED32>(
                 1, 21, input, this->mutable_arr())));
        } else if (::google::protobuf::internal::WireFormatLite::GetTagWireType(tag)
                   == ::google::protobuf::internal::WireFormatLite::
                      WIRETYPE_LENGTH_DELIMITED) {
          DO_((::google::protobuf::internal::WireFormatLite::ReadPackedPrimitiveNoInline<
                   ::google::protobuf::uint32, ::google::protobuf::internal::WireFormatLite::TYPE_FIXED32>(
                 input, this->mutable_arr())));
        } else {
          goto handle_uninterpreted;
        }
        if (input->ExpectTag(21)) goto parse_arr;
        if (input->ExpectAtEnd()) return true;
        break;
      }
      
      default: {
      handle_uninterpreted:
        if (::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_END_GROUP) {
          return true;
        }
        DO_(::google::protobuf::internal::WireFormat::SkipField(
              input, tag, mutable_unknown_fields()));
        break;
      }
    }
  }
  return true;
#undef DO_
}

void Ints::SerializeWithCachedSizes(
    ::google::protobuf::io::CodedOutputStream* output) const {
  // optional fixed32 data = 1;
  if (has_data()) {
    ::google::protobuf::internal::WireFormatLite::WriteFixed32(1, this->data(), output);
  }
  
  // repeated fixed32 arr = 2;
  for (int i = 0; i < this->arr_size(); i++) {
    ::google::protobuf::internal::WireFormatLite::WriteFixed32(
      2, this->arr(i), output);
  }
  
  if (!unknown_fields().empty()) {
    ::google::protobuf::internal::WireFormat::SerializeUnknownFields(
        unknown_fields(), output);
  }
}

::google::protobuf::uint8* Ints::SerializeWithCachedSizesToArray(
    ::google::protobuf::uint8* target) const {
  // optional fixed32 data = 1;
  if (has_data()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFixed32ToArray(1, this->data(), target);
  }
  
  // repeated fixed32 arr = 2;
  for (int i = 0; i < this->arr_size(); i++) {
    target = ::google::protobuf::internal::WireFormatLite::
      WriteFixed32ToArray(2, this->arr(i), target);
  }
  
  if (!unknown_fields().empty()) {
    target = ::google::protobuf::internal::WireFormat::SerializeUnknownFieldsToArray(
        unknown_fields(), target);
  }
  return target;
}

int Ints::ByteSize() const {
  int total_size = 0;
  
  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    // optional fixed32 data = 1;
    if (has_data()) {
      total_size += 1 + 4;
    }
    
  }
  // repeated fixed32 arr = 2;
  {
    int data_size = 0;
    data_size = 4 * this->arr_size();
    total_size += 1 * this->arr_size() + data_size;
  }
  
  if (!unknown_fields().empty()) {
    total_size +=
      ::google::protobuf::internal::WireFormat::ComputeUnknownFieldsSize(
        unknown_fields());
  }
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = total_size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
  return total_size;
}

void Ints::MergeFrom(const ::google::protobuf::Message& from) {
  GOOGLE_CHECK_NE(&from, this);
  const Ints* source =
    ::google::protobuf::internal::dynamic_cast_if_available<const Ints*>(
      &from);
  if (source == NULL) {
    ::google::protobuf::internal::ReflectionOps::Merge(from, this);
  } else {
    MergeFrom(*source);
  }
}

void Ints::MergeFrom(const Ints& from) {
  GOOGLE_CHECK_NE(&from, this);
  arr_.MergeFrom(from.arr_);
  if (from._has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    if (from.has_data()) {
      set_data(from.data());
    }
  }
  mutable_unknown_fields()->MergeFrom(from.unknown_fields());
}

void Ints::CopyFrom(const ::google::protobuf::Message& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

void Ints::CopyFrom(const Ints& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool Ints::IsInitialized() const {
  
  return true;
}

void Ints::Swap(Ints* other) {
  if (other != this) {
    std::swap(data_, other->data_);
    arr_.Swap(&other->arr_);
    std::swap(_has_bits_[0], other->_has_bits_[0]);
    _unknown_fields_.Swap(&other->_unknown_fields_);
    std::swap(_cached_size_, other->_cached_size_);
  }
}

::google::protobuf::Metadata Ints::GetMetadata() const {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::Metadata metadata;
  metadata.descriptor = Ints_descriptor_;
  metadata.reflection = Ints_reflection_;
  return metadata;
}


// @@protoc_insertion_point(namespace_scope)

}  // namespace test

// @@protoc_insertion_point(global_scope)
